...
SCC_table <- round(data.frame("rates" = rates,
                        "SCC" = SCC), 2)
...
```
  ...
  - Expected damages under Policy A: $`r scientific(policy_a_pv)`
  
  - Expected damages under Policy B: $`r scientific(policy_b_pv)`
  
  - Initial undertaking of Policy B would have to cost less than $`r scientific(difference_ab)` to justify choosing Policy A over Policy B
